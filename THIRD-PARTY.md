...
|---|---|---|---|
| React Bits — Strands | 주의 상태 엣지 글로우 · `strands-lab.html` | MIT + Commons Clause | **가능** · 조건 있음 |
...
- **쓰는 곳** — 두 곳입니다.
  - `assets/js/edge-strands.js` — 대시보드의 **주의(Warning) 상태 엣지 글로우**. 실제 화면에 쓰입니다.
...
엣지 글로우 쪽은 여기에 더해 **uv 좌표만 고쳤습니다** — 원본은 가닥이 화면을 가로질러 곧게
지나가는데, 테두리를 감아 돌도록 둘레 위치와 테두리 중심선에서 떨어진 거리로 바꿨습니다.
가닥을 쌓고 톤매핑하는 계산은 손대지 않았습니다.

저작권 표시는 `strands-lab.html` 과 `assets/js/edge-strands.js` 양쪽 머리에 들어 있습니다 —
...
판매용 빌드를 만들 때는 위 둘을 **제외**하는 편이 깔끔합니다.
...
- [x] Strands 를 실제 화면에 얹을 때 **저작권 주석이 함께 따라갔는지** — `edge-strands.js` 머리에 있음
