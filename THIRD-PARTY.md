...
- **원본과 다르게 둔 것**
  - **도는 빛을 박동으로 바꿨습니다.** 커서 원뿔 마스크와 두 변수 계산식을 빼고,
    네 변이 한꺼번에 2초에 두 번 뛰고 쉬도록 했습니다. 움직이는 것은 `opacity` 뿐입니다.
  - **빛을 두 줄로 나눠 사이에 8px 의 어두운 틈을 둡니다**(원본은 한 번에 잦아드는 한 겹입니다).
  - 번지는 거리는 **두 배**로 키웠습니다(`glowRadius` 40 → 80). 안쪽 줄의 흐림 반경에
    `--edge-glow-reach`(2)를 곱합니다.
