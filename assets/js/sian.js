...
(function () {
    ...
    return [
      {
        ...
        photo: true,   // assets/img/sian2-detect-frame.webp — 디자이너가 준 감지 프레임
        boxes: [
  ...
  var side1 = (function () {
    ...
    function render() {
      ...
      // Delta 문구는 목록에서 고른 알림을 따른다. 고른 것이 없으면(정상) 단계 기본 문구다.
      var chosen = all[picked];
      var copy = (chosen && chosen.delta) || DELTA_1[level];
      ...
      (copy.actions || []).forEach(function (label, i) {
        ...
        if (label === "자동복귀") { b.setAttribute("data-modal-open", "recall"); }
    ...
    // [로봇 정지] · [자동복귀] 는 screen.js 의 모달 여닫기에 물려야 해서, 새로 그린 버튼이 눌렸을 때 원래 트리거를 대신 누른다.
    actions.addEventListener("click", function (event) {
      var b = event.target.closest("[data-modal-open]");
      if (!b) { return; }
      var real = $(".rc-actions [data-modal-open='" + b.getAttribute("data-modal-open") + "']");
      if (real) { real.click(); }
    });
    ...
    document.addEventListener("aprism:estop", function () {
      ...
      extra.unshift({ sev: "critical", where: (chosen ? chosen.textContent : "Robot") + " · 현재 위치",
        ...
        delta: { line1: (chosen ? chosen.textContent : "로봇") + " 에 비상정지가 발동되었습니다.",
  ...
  var detail = (function () {
    ...
    function drawBoxes(a) {
      frame.classList.toggle("has-photo", !!a.photo);
